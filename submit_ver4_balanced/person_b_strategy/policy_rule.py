...
class RulePolicy:
    ...
    ``style`` selects the scoring temperament — ``"balanced"`` (default),
    ``"aggressive"``, or ``"safe"`` — which the per-phase profiles scale; this is
    the only thing that differs between the three submission versions.

    ``weights`` overrides the per-phase + style schedule with a single fixed
    ``ScoreWeights`` for every turn — used by the offline weight tuner
    (``train/tune_weights.py``); ``None`` keeps the shipped phase profiles.
    """

    def __init__(self, weights=None, style: str = "balanced"):
